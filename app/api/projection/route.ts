...
export async function POST(request: Request) {
  ...
  const slot = Math.max(0, Math.min(15, Math.floor(Number(payload.slot ?? 0))));
  ...
  const preview = Array.isArray(targetState?.preview) ? targetState.preview : [];
  const board = Array.isArray(targetState?.board) ? targetState.board : [];
  const event = slot < 4 ? preview[slot] : board[slot - 4];
  if (!senderState || !event) return jsonError("玩家棋盘尚未就绪");
  ...
  if (payload.faction === "angel" && slot >= 4 && createsMatch(board, slot - 4)) {
