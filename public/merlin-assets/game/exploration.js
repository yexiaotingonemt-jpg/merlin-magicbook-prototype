import { $, ELEMENTS, SCHOOL_ORDER, pick, shuffle } from "./core.js?v=10";
import { CARDS, CARD_BY_ID, PASSIVES } from "./cards.js?v=10";
import { runtime, setState, state } from "./store.js?v=10";
import { advanceChapter, bindCard, cardLevel, COMBAT_DECK_CAP, costLabel, freshTowerRun, gainExp, generateEvents, maxHp, saveState, settleExplorationTurn, slotCap } from "./state.js?v=10";
import { closeModal, render, showModal, showView, toast } from "./ui.js?v=10";
import { startBattle, stopBattle } from "./battle.js?v=10";
...
export function chooseCardModal(title, cards, action, copy = "选择一张书页。") {
  showModal(`<h2>${title}</h2><p>${copy}</p><p><b>已选定本事件：请做出选择以完成处理。</b></p><div class="choice-grid">${cards.map((card) => `<button class="choice-button ${card.school}" data-modal-card="${card.id}"><h3>${card.name}</h3><small>${card.id} · ${costLabel(card)}</small><p>${card.full}</p></button>`).join("")}</div>`, false);
...
export function choosePassiveModal() {
  ...
  showModal(`<h2>被动秘典 · 三选一</h2><p>被动卡不进入战斗魔法书，可无限升级；当所有战斗咒语满级后，成长牌库只会出现这些秘典。</p><p><b>已选定本事件：请做出选择以完成处理。</b></p><div class="choice-grid">${choices.map((passive) => `<button class="choice-button arcane" data-passive="${passive.id}"><h3>${passive.name}</h3><small>${passive.id} · 当前 Lv.${Number(state.meta.passiveLevels?.[passive.id] || 0)}</small><p>${passive.copy}</p></button>`).join("")}</div>`, false);
  $("modalContent").onclick = (event) => {
...
export function showElementEvent() {
  showModal(`<h2>元素池</h2><p>选择要增加的起始元素。${state.startElements.length >= slotCap() ? "当前已满，下一步需选择被替换的元素。" : ""}</p><p><b>已选定本事件：请完成元素编排。</b></p><div class="choice-grid">${SCHOOL_ORDER.slice(0, 6).map((e) => `<button class="choice-button ${e}" data-element="${e}"><h3>${ELEMENTS[e].icon} ${ELEMENTS[e].name}元素</h3><p>将${ELEMENTS[e].name}纳入起始编排。</p></button>`).join("")}</div>`, false);
  $("modalContent").onclick = (event) => {
    ...
    showModal(`<h2>选择替换位置</h2><p>选择一个现有元素，将它替换为${ELEMENTS[element].name}。</p><p><b>已选定本事件：请完成替换。</b></p><div class="choice-grid">${state.startElements.map((e, i) => `<button class="choice-button ${e}" data-replace="${i}"><h3>位置 ${i + 1}</h3><p>${ELEMENTS[e].icon} ${ELEMENTS[e].name} → ${ELEMENTS[element].icon} ${ELEMENTS[element].name}</p></button>`).join("")}</div>`, false);
