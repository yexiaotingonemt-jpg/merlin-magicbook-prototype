...
export function bindEvents() {
  document.addEventListener("click", (event) => {
    ...
    const archiveSchool = event.target.closest("[data-archive-school]")?.dataset.archiveSchool;
    if (archiveSchool) {
      $("archiveSchoolFilter").value = archiveSchool;
      $("archiveSchoolTabs").querySelectorAll("[data-archive-school]").forEach((button) => button.classList.toggle("active", button.dataset.archiveSchool === archiveSchool));
      renderArchive(); return;
    }
    const archiveCardId = event.target.closest("[data-archive-open]")?.dataset.archiveOpen || event.target.closest("[data-archive-card]")?.dataset.archiveCard;
    if (archiveCardId && !event.target.closest("[data-spell-term]")) {
      const card = CARD_BY_ID.get(archiveCardId); if (card) showModal(archiveDetailHtml(card)); return;
    }
  ...
  ["archiveSearch", "archiveSchoolFilter"].forEach((id) => $(id).addEventListener(id === "archiveSearch" ? "input" : "change", () => {
    $("archiveSchoolTabs").querySelectorAll("[data-archive-school]").forEach((button) => button.classList.toggle("active", button.dataset.archiveSchool === $("archiveSchoolFilter").value));
    renderArchive();
  }));
